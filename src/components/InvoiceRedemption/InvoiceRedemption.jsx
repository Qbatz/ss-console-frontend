...
const InvoiceRedemption = () => {
  ...
  const fetchData = async () => {
    const res = await getInvoiceRedemption(
      page - 1, // 🔥 API starts from 0
      rowsPerPage,
      search
    );

    if (res.success) {
      const list = res.data?.content || res.data?.invoiceList || [];
      setData(list);
                ...
                {loading ? (
                  ...
                  data.map((item, i) => (
                    <tr key={i} className="border-t hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2">{item.sourceInvoiceNumber}</td>
                      <td className="px-3 py-2">{item.targetInvoiceNumber}</td>
                      <td className="px-3 py-2">{item.hostelName}</td>
                      <td className="px-3 py-2">{item.redemptionAmount}</td>
