...
const TableCustomization = () => {
  const { getTableColumns, updateTableColumns, resetTableColumns, loading,getTableColumnFilterOptions,updateTableColumnFilterOption } = useHostel();
  ...
  const [activeColumnTab, setActiveColumnTab] = useState("USER");
const [defaultColumns, setDefaultColumns] = useState([]);
const [defaultTotalRecords, setDefaultTotalRecords] = useState(0);
...
const handleSave = async () => {
  try {
    ...
    if (activeColumnTab === "DEFAULT") {
      const payload = {
        moduleName: selectedRow?.moduleName,
        columns: selectedRow?.columns?.map((col, index) => ({
          order: index + 1,
          fieldName: col?.fieldName,
          isSelected : Boolean(col?.selected),
        })),
      };

      console.log(
        "DEFAULT FILTER OPTION ID:",
        selectedRow?.filterOptionId
      );
      ...
      if (!selectedRow?.filterOptionId) {
        setTableError("Filter Option ID is missing");
        return;
      }

      const res = await updateTableColumnFilterOption(
        selectedRow.filterOptionId,
        payload
      );

      console.log("DEFAULT PUT RESPONSE:", res);

      if (res.success) {
        setModalType("success");
        setMessage("Default columns updated successfully");
        setShowSuccess(true);
        setTableError("");

        await fetchDefaultColumns();
        ...
        setTableError(
          res.message || "Failed to update default columns"
        );
      }
...
const fetchDefaultColumns = async () => {
  try {
    ...
    const res = await getTableColumnFilterOptions(
      apiPage,
      rowsPerPage,
      debouncedSearch
    );

    console.log("Default Columns Response:", res);

    if (res.success) {
      setDefaultColumns(res?.data?.filterOptions || []);

      setDefaultTotalRecords(
        res?.data?.totalItems || 0
      );

      setDefaultTotalPages(
        res?.data?.totalPages || 0
      );
    } else {
      setDefaultColumns([]);
      setDefaultTotalRecords(0);
      setDefaultTotalPages(0);
    }
  } catch (error) {
    console.error("Default Columns Fetch Error:", error);
...
<div className="flex border-b border-gray-200 mb-4">
  ...
  <button
    onClick={() => {
    setActiveColumnTab("DEFAULT");
    setPage(1);
    setTableError("");
  }}
    className={`px-5 py-3  ${
      activeColumnTab === "DEFAULT"
        ? "text-blue-600 border-b-2 border-blue-600"
        : "text-gray-500 cursor-pointer"
    }`}
  >
        ...
        <td className="px-6 py-4">
          ...
          <div
            onClick={() => {
              ...
              setSelectedRow({
                filterOptionId: item.filterOptionId,
                moduleName: item.moduleName,
                columns: item.filterOptions || [],
              });
